...
public class DatabaseManager {
    ...
    private int xMouse, yMouse; //لتخزين احداثيات الماوس عند السحب
    ...
    public boolean registerUser(String userId, String name, String email,
        ...
        try {
            ...
            // 2. أضف في جدول users (بدون country)
            String sql = "INSERT INTO users (user_id, name, email, password, phone_number, role) " +
                    "VALUES (?, ?, ?, ?, ?, ?)";
            ...
            // 3. أضف في جدول customers أو admins
            if (role.equals("عميل")) {
                String custSql = "INSERT INTO customers (customer_id, shipping_address, country) " +
                        "VALUES (?, ?, ?)";
                PreparedStatement custStmt = conn.prepareStatement(custSql);
                custStmt.setString(1, userId);
                custStmt.setString(2, "");      // عنوان فارغ في البداية
                custStmt.setString(3, country);
                custStmt.executeUpdate();
            } else {
                String admSql = "INSERT INTO admins (admin_id, admin_level, department) " +
                        "VALUES (?, 'REGULAR', 'General')";
                PreparedStatement admStmt = conn.prepareStatement(admSql);
                admStmt.setString(1, userId);
                admStmt.executeUpdate();
            }
