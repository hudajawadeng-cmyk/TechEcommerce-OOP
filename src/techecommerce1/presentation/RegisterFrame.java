...
public class RegisterFrame extends JFrame {
    ...
    private void initComponents() {
        ...
        JPanel root = new JPanel(new BorderLayout(0, 14)) {
            @Override protected void paintComponent(Graphics g) {
                ...
                GradientPaint gp = new GradientPaint(0, 0, BG_DARK, getWidth(), getHeight(), new Color(235,238,245));
                g2.setPaint(gp);
        ...
        addRow(card, gc, 1, "الاسم الكامل",         nameField);
        addRow(card, gc, 2, "البريد الالكتروني",              emailField);
        addRow(card, gc, 3, "كلمة المرور",           passwordField);
        addRow(card, gc, 4, "تأكيد كلمة المرور",   confirmField);

        // ── Separator
        addSeparator(card, gc, 5);

        // ── Section: Personal Info
        addSectionLabel(card, gc, 6, "البيانات الشخصية  (اختياري)");
        ...
        addRow(card, gc, 7,  "الهاتف",    phoneField);
        addRow(card, gc, 8,  "الدولة",  countryBox);
        addRow(card, gc, 9,  "الصلاحية",     roleBox);
    ...
    private void handleRegister() {
        ...
        if (name.isEmpty() || email.isEmpty() || password.isEmpty()) {
            setStatus("⚠  الاسم, البريد الالكتروني, وكلمة المرور مطلوبة.", ERROR_C);
            return;
        }
        if (!email.contains("@") || !email.contains(".")) {
            setStatus("⚠  يرجى ادخال بريد الكتروني صحيح.", ERROR_C);
            return;
        }
        if (password.length() < 6) {
            setStatus("⚠  يجب أن تتكون كلمة المرور من 6 خانات على الأقل.", ERROR_C);
            return;
        }
        if (!password.equals(confirm)) {
            setStatus("✖  كلمتا المرور غير متطابقتين.", ERROR_C);
            return;
        }
        ...
        if (db.isConnected()) {
            ...
            if (!success) {
                setStatus("✖  البريد الالكتروني مسجل بالفعل أو حدث خطأ في قادة البيانات.", ERROR_C);
                return;
            }
        }

        // ── Success
        setStatus("✔  تم انشاء الحساب بنجاح!", SUCCESS_C);
        JOptionPane.showMessageDialog(this,
                "🎉 أهلاً بك, " + name + "!\n\nتم انشاء حسابك بنجاح.\nيمكنك الاّن تسجيل الخول الى النظام.",
                "تم التسجيل بنجاح", JOptionPane.INFORMATION_MESSAGE);
        dispose();
    }
    ...
    private JTextField darkField(int cols) {
        ...
        f.setBackground(new Color(235,238,244));
    ...
    private void styleCombo(JComboBox<String> box) {
        box.setBackground(new Color(235,238,244));
        box.setForeground(TEXT_DIM);
        box.setFont(new Font("Monospaced", Font.PLAIN, 14));
