...
public class TrackShipmentFrame extends JFrame {
    ...
    private void initComponents() {
        ...
        title.setForeground(new Color(20, 100, 210));
        ...
        orderLbl.setFont(new Font("Monospaced", Font.PLAIN, 13));

        String[] orderIds = new String[SHIPMENTS.length];
        for (int i = 0; i < SHIPMENTS.length; i++)
            orderIds[i] = (String) SHIPMENTS[i][0];
        orderBox = new JComboBox<>(orderIds);
        orderBox.setBackground(new Color(255,255,255));
        orderBox.setForeground(TEXT_DIM);
        orderBox.setFont(new Font("Monospaced", Font.PLAIN, 14));
        ...
        trackLbl.setFont(new Font("Monospaced", Font.PLAIN, 13));

        trackingField = new JTextField(14);
        trackingField.setBackground(new Color(255,255,255));
        trackingField.setForeground(TEXT_MAIN);
        trackingField.setCaretColor(ACCENT);
        trackingField.setFont(new Font("Monospaced", Font.PLAIN, 14));
        ...
        infoRow.add(infoCard("Status",  statusBadge,  new Color(0,130,190)));
        infoRow.add(infoCard("ETA",     etaLabel,     new Color(0,140,90)));
        infoRow.add(infoCard("Carrier", carrierLabel, new Color(120,60,200)));
        infoRow.add(infoCard("Origin",  originLabel,  new Color(200,120,0)));
        ...
        evTitle.setFont(new Font("Monospaced", Font.BOLD, 14));
        ...
        destLabel.setFont(new Font("Monospaced", Font.PLAIN, 13));
    ...
    private JPanel infoCard(String label, JLabel valueLbl, Color accent) {
        ...
        valueLbl.setFont(new Font("Monospaced", Font.BOLD, 14));
    ...
    private void drawTimeline(Graphics2D g2) {
        ...
        g2.setColor(new Color(210,215,226));
        ...
        for (int i = 0; i < n; i++) {
            ...
            g2.setColor(done?new Color(0,140,90):current?new Color(0,110,190):new Color(120,130,150));
            String name = STEPS[i];
            ...
            fm = g2.getFontMetrics();
            g2.setColor(future ? new Color(170,178,195) : new Color(90,105,130));
    ...
    private void loadShipment(int idx) {
        ...
        switch (status) {
            ...
            case "Out for Delivery": statusBadge.setForeground(new Color(0,140,180)); break;
            case "In Transit":       statusBadge.setForeground(new Color(30,110,220)); break;
            case "Dispatched":       statusBadge.setForeground(new Color(120,60,200)); break;
            default:                 statusBadge.setForeground(new Color(180,120,0));
        }
        ...
        for (int i = showUntil-1; i >= 0; i--) {
            String col = (i == showUntil-1) ? "#0A66C2" : "#5A6B85";
            html.append(String.format(
                    "<tr><td style='color:#455066;font-size:12px;white-space:nowrap;'>%s</td>"
                            +"<td style='color:%s;font-size:13px;padding-left:12px;'>%s</td>"
                            +"<td style='color:#223047;font-size:12px;padding-left:12px;'>%s</td></tr>",
